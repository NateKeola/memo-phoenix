...
- `canonical_layer`: `{ "nodes": [ { "id", "label", "summary", "source_claim_ids" } ], "relationships": [ { "id", "label", "summary" } ] }`. The `source_claim_ids` on each node are claim handles: your provenance handles. Each is a short opaque code (4 characters, letters and digits), not a uuid. Copy them back exactly as given.
...
2. Ground every insight. `supporting_claim_ids` MUST list the handles (drawn from the `source_claim_ids` you were given) that underpin the finding, at least one. `affected_entity_ids` lists the canonical node ids the finding is about.
  ...
  "insights": [
    {
      ...
      "supporting_claim_ids": ["<handle from the layer's source_claim_ids>"],
      "affected_entity_ids": ["<canonical node id>"],
      "confidence": 0.0
    }
  ],
  "discrepancies": [ { "subject": "<topic>", "description": "<conflict>", "claim_ids": ["<handle>", "<handle>"] } ],
