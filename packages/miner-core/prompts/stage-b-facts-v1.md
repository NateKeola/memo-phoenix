...
- `claims`: a JSON array of `{ "id": "<handle>", "data": { ...the extracted fact... } }`. The `id` is your provenance handle: a short opaque code (4 characters, letters and digits), not a uuid. Copy it back exactly as given.
...
```
{
  ...
  "discrepancies": [ { "subject": "<fact>", "description": "<conflict>", "claim_ids": ["<handle>", "<handle>"] } ],
